...
tic;
for J = max([2,ceil(log2(options.MinNumStates))]):ceil(log2(options.MaxNumStates))
    mindist = Inf;
    best = 0;
    bestidx = 0;
    
    randomruns = ceil(2*options.Runs/3);
...
return
    ...
    function dist = dist_fun(Eref,Eapx)
        ...
        dist = w*abs( log10(Eref) - log10(Eapx) )';
    end
