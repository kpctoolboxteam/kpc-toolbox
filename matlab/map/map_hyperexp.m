...
if map_isfeasible(MAP) % if the first root is feasible
    return
else % if the first root is not feasible go for the second root
    mu2=(-2*MEAN+2*p*MEAN-sqrt(Delta))/(E2*p-2*MEAN^2);
    mu1=mu2*p/(p-1+MEAN*mu2);
    MAP={[-mu1,0;0,-mu2],[mu1*p,mu1*(1-p);mu2*p,mu2*(1-p)]};
    if map_isfeasible(MAP) % if the second root is feasible
        return
    elseif p>1e-6 % if the solution is not feasible try to decrease p
